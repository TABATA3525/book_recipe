  ...
  <div>
    <div class="star-container" v-for="(feeling, index) in feelingCategories" :key="index" >
      ...
      <v-btn 
        ...
        color="primary"
        type="button"
        @click.prevent="addFeelingCategory"
      >
      ...
      <v-btn 
        ...
        type="button"
        v-if="index !== 0"
        @click.prevent="deleteFeelingCategory(index)"
      >
  ...
  methods: {
    setRating: function(){
      ...
      this.star.rating.forEach(function(element,index){
        ...
        const input = Array.isArray(refs[property]) ? refs[property][0] : refs[property]
